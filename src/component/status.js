...
const Status = () => {
  const sendEmail = async (data, id) => {
    ...
    urlencoded.append("subject", `Delivery System  `);
    urlencoded.append(
      "body",
      `<html>
          <link
  ...
  React.useEffect(async () => {
    await getData().then(async (res) => {
      ...
      if (res.status === "waiting") {
        await updateStatus(id, status).then(async (r) => {
          if (status === "approve") {
  ...
  if (reqData && reqData.status === "waiting") {
    return (
      <>
        <Backdrop open={true}>
          <Card
            style={{ marginTop: 24, marginLeft: "8vw", marginRight: "8vw" }}
          >
            <CardContent>
        ...
        <Card style={{ marginTop: 24, marginLeft: "8vw", marginRight: "8vw" }}>
          <CardContent>
            <Alert severity="error">
              This request is already {reqData && reqData.status}
            </Alert>
            {/* <h2></h2> */}
          </CardContent>
        </Card>
